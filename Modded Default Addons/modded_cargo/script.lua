	...
	if tableLength(g_cargo_terminals) == 0 then
		...
		for cargo_terminal_index_new, cargo_terminal_object_new in pairs(g_cargo_terminals) do
			if #g_cargo_package_types > 0 then
				local cargo_to_spawn_count = math.random(math.ceil(#cargo_terminal_object_new.spawn_locations / 2),
					#cargo_terminal_object_new.spawn_locations)
						...
						if cargo_terminal_dest ~= nil then
							for delivery_zone_index, delivery_zone_object in pairs(cargo_terminal_dest.delivery_zones) do
								if matrix.distance(cargo_vehicle_transform, delivery_zone_object.transform) < 200 then
									if server.isInTransformArea(cargo_vehicle_transform, delivery_zone_object.transform, delivery_zone_object.size.x
										, delivery_zone_object.size.y, delivery_zone_object.size.z) then
										server.notify(-1, "Cargo Delivered",
											"Cargo has been delivered for a reward of $" .. cargo_package_object.reward .. ".", 4)
...
function spawnCargo(cargo_terminal)
	...
	for spawn_location_index, spawn_location_object in pairs(cargo_terminal.spawn_locations) do
		if spawn_location_object.cargo_package_exists == false and
			server.isLocationClear(spawn_location_object.transform, 5, 7, 7) then -- check location is clear
	...
	if #valid_spawn_locations > 0 then
		...
		if #valid_cargo_package_types > 0 then
			...
			if #valid_destination_cargo_terminals > 0 then
				local selected_destination_cargo_terminal = valid_destination_cargo_terminals[
					math.random(1, #valid_destination_cargo_terminals)]

				-- calculate reward
				local transport_distance = matrix.distance(cargo_terminal.transform, selected_destination_cargo_terminal.transform)
				local reward_value = math.ceil((transport_distance / 10) * (math.random(10, 40) / 10) / 1000) * 1000 *
					math.ceil(selected_cargo_package.probability / 1.0)
				...
				for _, object_data in iterObjects(cargo_package_object.type.playlist_index, cargo_package_object.type.location_index) do
					if object_data.type == "vehicle" then
						if cargo_package_object.vehicle_id == nil then
							...
							local component = server.spawnAddonComponent(spawn_transform, cargo_package_object.type.playlist_index,
								cargo_package_object.type.location_index, object_data.index, -1)
